...
exports.signUp = async (req, res) => {
  ...
  // Check if the email address is from gitam domain
  if (!email.endsWith("@gitam.in") && !email.endsWith("@gitam.com")) {
    return res.status(400).json({ msg: "Only Gitam students can register" });
  }
  ...
  try {
    ...
    updatedUser.save();
    const sender = "bookbuddy@gmail.com";
...
exports.sendMail = async (req, res) => {
  ...
  try {
    ...
    const receiver = "bookbuddy@gmail.com";
...
const sendGoogleMail = async (to, toName, password) => {
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      requireTLS: true,
      auth: {
        user: "bookbuddy@gmail.com",
        pass: "Book@123123",
      },
    });

    const mailOptions = {
      from: "bookbuddyr@gmail.com",
      to: receiver,
      subject: subject,
      text: message,
    };
